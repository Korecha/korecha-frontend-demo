...
import { registerExporter } from '../../api/register'
...
export function RegisterExporterPage() {
  ...
  const [exportLicense, setExportLicense] = useState<File | null>(null)

  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault()
    if (!nationalId || !exportLicense) {
      setError('National ID and export license are required')
      return
    }
    setSubmitting(true)
    setError('')
    try {
      ...
      fd.append('exportLicense', exportLicense)
      const res = await registerExporter(fd)
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-white px-6 py-10">
      <div className="mx-auto max-w-2xl">
        <PageHeader
          ...
          description="Register as an independent exporter. A platform admin will review your documents before you can use the portal."
        />
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <Field label="Export license">
              <Input
                type="file"
                accept="image/*,.pdf"
                onChange={(e) => setExportLicense(e.target.files?.[0] || null)}
                required
              />
            </Field>
