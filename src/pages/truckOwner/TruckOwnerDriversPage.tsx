...
export function TruckOwnerDriversPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await createTruckOwnerDriver(fd)
      setShowForm(false)
      resetForm()
      load()
    } catch (err) {
  ...
  if (!approved) {
    return (
      <div>
        <PageHeader title="Drivers" description="Hire and manage your drivers" />
        ...
        <Table>
          ...
          <tbody>
            {loading ? (
              ...
              <TableEmpty colSpan={5} message="No drivers yet. Add your first driver." />
            ) : (
              drivers.map((d) => (
                <TableRow key={d.id}>
