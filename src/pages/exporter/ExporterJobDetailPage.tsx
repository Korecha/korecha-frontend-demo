    ...
    >
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 text-korecha-primary">
        ...
        >
          <path
            ...
            d="M8 17h8M5 11h14l-1.5-5H6.5L5 11zm2.5 6a1.5 1.5 0 100-3 1.5 1.5 0 000 3zm7 0a1.5 1.5 0 100-3 1.5 1.5 0 000 3z"
          />
        </svg>
      </div>
      <div className="min-w-0 flex-1">
        ...
        <p className="text-sm text-slate-600">
          ...
          <span
            className={
              extended ? 'font-semibold text-amber-700' : 'font-medium text-korecha-primary'
            }
          >
...
export function ExporterJobDetailPage() {
  ...
  if (!organization) {
    return (
      <Alert variant="warning">
        Your account is approved. Contact the platform admin to be linked to an organization before
        viewing jobs.
      </Alert>
    )
  }
  ...
  const showPricing =
    job.pricingQuote &&
    !['ASSIGNED', 'IN_TRANSIT', 'PENDING_APPROVAL', 'COMPLETED'].includes(job.status)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-3xl border border-korecha-border bg-white p-5 shadow-sm">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <div className="rounded-xl bg-blue-50 px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-wide text-korecha-primary">
              Pickup
            </p>
          ...
          <div className="rounded-xl bg-amber-50 px-3 py-2.5">
            ...
            {deliveryGate && (
              <p className="mt-1 text-xs text-slate-500">Gate: {deliveryGate.name}</p>
            )}
          </div>
        </div>
        {job.notes && (
        ...
        {driver && (
          <div className="mt-4 flex items-center justify-between rounded-2xl bg-blue-50 px-4 py-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-korecha-primary">
                Assigned driver
              </p>
      ...
      {awaitingApproval && (
        <div className="rounded-3xl border border-orange-200 bg-gradient-to-br from-orange-50 to-white p-5 shadow-sm">
          ...
          <p className="mt-1 text-sm text-orange-800">
            Review the job and approve to finalize. The driver delivered on{' '}
      ...
      {job.status === 'COMPLETED' &&
        (payment ? (
          <Card className="border-emerald-200 bg-gradient-to-br from-emerald-50 to-white">
            <h3 className="font-bold text-emerald-900">Payment Details</h3>
      ...
      {showPricing && job.pricingQuote && <JobPricingCard quote={job.pricingQuote} />}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Live drivers from all organizations within ~{nearbyRadiusKm} km
              </p>
            </div>
          </div>
          ...
          {nearby.length === 0 && extended.length === 0 ? (
            <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-6 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Drivers appear when they go live and set availability
              </p>
            </div>
          ) : nearby.length > 0 ? (
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <p className="text-sm font-semibold text-amber-900">
                  No trucks within {nearbyRadiusKm} km
                </p>
