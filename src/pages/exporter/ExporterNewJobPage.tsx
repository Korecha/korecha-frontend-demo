...
export function ExporterNewJobPage() {
  ...
  useEffect(() => {
    ...
    void Promise.resolve()
      .then(() => {
        ...
        return listNearbyAvailabilityPostings({ lat: coords.lat, lng: coords.lng })
      })
      .then((r) => {
        if (cancelled || !r) return
        setNearbyAvailability(r.data)
      })
      .catch(() => {
        if (!cancelled) setNearbyAvailability([])
      })
      .finally(() => {
  ...
  }, [pickupLocation?.id, pickupLocation?.coordinates])
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (posting.matchingMode === 'MANUAL_REQUEST') {
        ...
        navigate(jobId ? `/exporter/jobs/${jobId}` : `/exporter/load-postings/${posting.id}`)
        return
      }
      setSuccess(`Load posted and broadcast (${posting.mode}).`)
      navigate(`/exporter/load-postings/${posting.id}`, { replace: true })
    } catch (err) {
  ...
  if (!approved) {
    return <Alert variant="warning">Post jobs after your account is approved.</Alert>
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-3xl border border-korecha-border bg-white p-5 shadow-sm">
        <h2 className="text-xl font-bold text-slate-900">Post an export haul job</h2>
      ...
      <form
        ...
        className="space-y-4 rounded-3xl border border-korecha-border bg-white p-5 shadow-sm"
      >
        ...
        <div>
          <p className="mb-2 text-sm font-medium text-slate-700">
            How should fleets find this load?
          </p>
