...
export function ExporterHomePage() {
  const { memberProfile, organization } = useAuth()
  const approved = isApproved(memberProfile)
  const isSoleExporter = !organization
  const canUseJobs = approved && !isSoleExporter
    ...
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-korecha-navy via-korecha-navy-light to-blue-600 p-6 text-white shadow-xl shadow-blue-900/20">
        ...
        <div className="relative">
          ...
          <p className="mt-2 max-w-sm text-sm text-blue-100/90">
            Post export jobs, find live trucks nearby, and track every delivery until it&apos;s
            done.
          </p>
          {approved && !isSoleExporter ? (
            <Link to="/exporter/jobs/new" className="mt-5 inline-block">
            ...
            <p className="mt-4 rounded-xl bg-white/10 px-3 py-2 text-xs font-medium text-blue-100">
              Your account is approved. Contact the platform admin to be linked to an organization
              before posting jobs.
            </p>
          ) : (
            <p className="mt-4 rounded-xl bg-white/10 px-3 py-2 text-xs font-medium text-blue-100">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <div
            key={s.key}
            className="rounded-2xl border border-korecha-border bg-white p-4 text-center shadow-sm"
          >
          ...
          {recentJobs.length === 0 ? (
            <div className="rounded-3xl border border-dashed border-blue-200 bg-white p-10 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Post your first export haul request to find trucks
              </p>
              <Link to="/exporter/jobs/new" className="mt-4 inline-block">
