...
const navItems = [
  { to: '/corporate', label: 'Overview', end: true },
  { to: '/corporate/loads', label: 'My Loads', end: false },
  { to: '/corporate/loads/new', label: 'Post Load', end: false },
]
...
export function CorporateLayout() {
  ...
  const companyName =
    memberProfile?.type === 'corporate'
      ? (memberProfile.profile as CorporateCustomerProfile).companyName
      : null
    ...
    <div className="flex min-h-screen bg-korecha-bg">
      <aside className="fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-korecha-border bg-white shadow-sm">
        <div className="border-b border-korecha-border px-6 py-6">
          <h1 className="truncate text-base font-bold text-slate-900">{companyName || user?.fullName || 'Corporate'}</h1>
